...
fire_cascade = cv2.CascadeClassifier(
    'fire_detection_cascade_model.xml')  # To access xml file which includes positive and negative images of fire. (Trained images)
...
def play_alarm_sound_function():  # defined function to play alarm post fire detection using threading
    playsound.playsound(r"C:\Users\younl\Downloads\file_example_MP3_700KB.mp3", True)  # to play alarm # mp3 audio file is also provided with the code.
...
def send_mail_function():  # defined function to send mail post fire detection using threading
    ...
    recipientmail = "ajmrhw12@gmail.com"  # recipients mail
    ...
    try:
        ...
        server.ehlo()
        server.starttls()
        server.login("younliacmh@gmail.com", 'password')  # Senders mail ID and password
        server.sendmail('younliacmh@gmail.com', recipientmail,
                        "Warning fire accident has been reported")  # recipients mail with mail message
